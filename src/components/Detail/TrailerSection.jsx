...
const TrailerVideo = ({ video, index }) => {
  ...
  return (
    ...
    >
      <iframe
        ...
        title={video.name}
        className="w-full h-64 md:h-80 rounded-lg shadow-2xl border-0"
...
const TrailerSection = ({ data, type = "movie" }) => {
  ...
  const contentTypeText = type === "tv" ? "TV 프로그램" : "영화";
      ...
      {videos.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
          {videos.slice(0, 2).map((video, index) => (
            <TrailerVideo key={video.id || video.key} video={video} index={index} />
          ))}
        </div>
      ) : (
        <div className="text-center text-gray-400 text-lg mb-10 py-8">
          이 {contentTypeText}의 예고편을 찾을 수 없습니다.
        </div>
      )}
    </div>
  );
